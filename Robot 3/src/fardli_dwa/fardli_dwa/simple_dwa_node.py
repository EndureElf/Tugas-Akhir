...
class HeadingAlignedDWATracker(Node):
    def __init__(self):
        ...
        self.declare_parameter('path_tracking_tolerance', 0.01)
        ...
        self.declare_parameter('max_sideways_ratio', 0.3)  # Max sideways/forward ratio
    ...
    def update_parameters(self):
        ...
        self.get_logger().debug(f"Updated params: vx={self.max_vel_x:.2f}, vy={self.max_vel_y:.2f}, w={self.max_rot_vel:.2f}")
    ...
    def update_tracking_position(self):
        ...
        search_end = min(len(self.global_path) - 1, self.current_path_index + 3)
    ...
    def control_loop(self):
        ...
        if self.final_goal_pose:
            ...
            if dist_to_goal < self.goal_tolerance and not self.position_reached:
                ...
                self.get_logger().info("📍 POSITION GOAL REACHED! Starting final orientation alignment...")
                # Switch to final alignment state
                self.current_state = self.STATE_FINAL_ALIGNING
                # Use final orientation as target
                self.target_heading = self.goal_orientation
        
        # Determine behavior state if not already in final alignment
        if not self.position_reached:
            self.determine_state()
        else:
            # Once position reached, stay in final alignment until orientation is correct
            final_heading_error = abs(self.normalize_angle(self.goal_orientation - robot_theta))
            if final_heading_error <= self.heading_alignment_tolerance:
                self.get_logger().info("🎉 GOAL COMPLETELY REACHED! (Position and Orientation)")
                self.stop_robot()
    ...
    def determine_state(self):
        """Determine current behavior state - FOR PATH FOLLOWING ONLY"""
        robot_x, robot_y, robot_theta = self.robot_pose
    ...
    def tracking_control(self):
        """DWA for path following - IGNORE FINAL ORIENTATION UNTIL POSITION REACHED"""
        robot_x, robot_y, robot_theta = self.robot_pose
        target_x, target_y = self.target_point
        ...
        for vx in vx_samples:
            for vy in vy_samples:
                ...
                # Limit sideways when heading is good
                ...
                for w in w_samples:
                    ...
                    # Score this motion - PATH FOLLOWING ONLY, NO FINAL ORIENTATION
    ...
    def calculate_path_following_score(self, pred_x, pred_y, pred_theta,
                                      ...
                                      robot_x, robot_y, robot_theta,
                                      heading_error):
        """Score for path following ONLY - no final orientation consideration"""
        ...
        # NO FINAL ORIENTATION SCORE HERE - only after position reached
